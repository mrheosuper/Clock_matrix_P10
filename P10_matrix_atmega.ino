...
#define SCLK 6
#define OE 5
//SPIClass SPI(2); //Create an SPI2 object.
RTC_DS3231 rtc;
DateTime now;
...
void setup()
{
  ...
  pinMode(SCLK,OUTPUT);
  pinMode(OE,OUTPUT);
  SPI.begin(); //Initiallize the SPI 2 port.
  //SPI.setBitOrder(MSBFIRST); // Set the SPI-2 bit order (*) 
  //SPI.setDataMode(SPI_MODE0); //Set the  SPI-2 data mode (**) 
  //Set the SPI speed
  SPI.setClockDivider(SPI_CLOCK_DIV2);    // Slow speed (16 / 2 = 8 MHz SPI speed) 
  rtc.begin();
}
void loop()
{ 
...
void print_matrix(int32_t * data)
{
  for(int i=0;i<4;i++)
  {
    ...
    {
    SPI.transfer(~data[i+12]>>j);
    SPI.transfer(~data[i+8]>>j);
    SPI.transfer(~data[i+4]>>j);
...
void clear_matrix()
{
  digitalWrite(OE,0);
  digitalWrite(SCLK,0);
  for(int j=0;j<16;j++) SPI.transfer(0xff);
  digitalWrite(OE,1);
  digitalWrite(SCLK,1);
}
